...
{
    int ret = 0;
    wrapper_camera2_device_t *wrapper_dev = (wrapper_camera2_device_t *) device;
                ...
                hw_device_t** device)
{
    ...
    if (name != NULL) {
        ...
        camera2_device = (wrapper_camera2_device_t*) calloc(1, sizeof(*camera2_device));
        if (!camera2_device) {
            ALOGE("camera2_device allocation fail");
            rv = -ENOMEM;
            goto fail;
        }
        camera2_device->camera_released = false;
        ...
        camera2_ops = (camera_device_ops_t*) calloc(1, sizeof(*camera2_ops));
        if (!camera2_ops) {
