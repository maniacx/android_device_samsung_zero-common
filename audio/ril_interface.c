...
#define RIL_UNSOL_SNDMGR_CLOCK_CTRL 11022
...
static ril_pcm_clock_callback _clock_state_callback;
static void *_clock_state_data = NULL;
...
static int ril_internal_pcm_clock_callback(HRilClient client __unused,
                                        const void *data,
                                        size_t datalen)
{
    int clock_state = 0;

    if (_clock_state_data == NULL || _clock_state_callback == NULL) {
        return -1;
    }
    ...
    clock_state = *((int *)data);

    _clock_state_callback(_clock_state_data, clock_state);
...
int ril_set_pcm_clock_callback(struct ril_handle *ril,
                            ril_pcm_clock_callback fn,
                            void *data)
{
    int rc;
    ...
    _clock_state_callback = fn;
    _clock_state_data = data;

    /* register the pcm CLOCK callback */
    rc = RegisterUnsolicitedHandler(ril->client,
                                    RIL_UNSOL_SNDMGR_CLOCK_CTRL,
                                    (RilOnUnsolicited)ril_internal_pcm_clock_callback);
    if (rc != RIL_CLIENT_ERR_SUCCESS) {
        ril_close(ril);
        return -1;
    }
